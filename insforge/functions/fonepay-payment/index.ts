  ...
  ;(async () => {
    try {
      const { data: booking } = await db
        .from("bookings")
        .select("id, guest_name, guest_email, guest_phone, check_in, check_out, room_id, total_price, advance_amount, balance_amount, booking_status, payment_status, adults, created_at, special_requests")
        .eq("id", bookingId)
        .single()
      if (!booking) return
      ...
      const info: BookingInfo = {
        ...
        payment_status: "paid",
        guests: booking.adults || undefined,
